...
def detect_pop_voicegroup(root):
    # Current HLW projects renamed the old "abracadabra" bank to "diva_pop".
    # Prefer the current name, but keep compatibility with older copies.
    for name in ("diva_pop", "pop", "abracadabra"):
        p = find_vg(root, name)
        if p is not None:
            return name, p

    # Last-resort diagnostic: show POP-like group names that actually exist.
    found = []
    for p in (root / "sound").rglob("*.inc"):
        try:
            s = p.read_text(errors="ignore")
            ...
            continue
        for m in re.finditer(r'^\s*voice_group\s+([A-Za-z0-9_]+)\s*$', s, re.M):
            name = m.group(1)
            if "pop" in name.lower() or "diva" in name.lower() or "abra" in name.lower():
                found.append(name)

    found = sorted(set(found))
    extra = ""
    if found:
        extra = "\nGrupos parecidos encontrados: " + ", ".join(found)

    die(
        "Nao achei nenhum voice_group compativel "
        "(tentei: diva_pop, pop, abracadabra)." + extra
    )
...
def main():
    ...
    group_name, vg = detect_pop_voicegroup(root)
    vv=voices(vg.read_text(errors="ignore"))
    expected={1:"synth_bass",2:"nylon",5:"pizzicato",7:"piano",8:"pink_and_white_drumset"}
    print("== HALO + DISTURBIA + UMBRELLA FIX V2 / AUTO POP VOICEGROUP ==")
    print("Voicegroup detectado:", group_name)
    print("Arquivo:", vg)
    ...
    cfgtext,dline=set_cfg_group(cfgtext,"mus_disturbia_rihanna",group_name,"12","090")
    cfgtext,uline=set_cfg_group(cfgtext,"mus_umbrella_rihanna",group_name,"12","090")
