...
def detect_lullabies_assets(root):
    ...
    pngs = [p for p in files if is_lullabies_png(p)]

    if len(pngs) != 1:
        print("\nPNGs candidatos Lullabies:")
        for p in files:
            n = norm(p.stem)
            if p.suffix.lower() == ".png" and ("lullabies" in n or "paraly" in n):
                print(" ", p.name)
        die(f"Esperava exatamente 1 PNG de Lullabies to Paralyze; achei {len(pngs)}.")

    # IMPORTANT:
    # pokeemerald-expansion's graphics build generates the converted outputs
    # from the source PNG. They do NOT need to exist before make.
    #
    # Example:
    #   lullabiestoparalyze.png
    #       -> lullabiestoparalyze.gbapal
    #       -> lullabiestoparalyze.4bpp.smol
    png = pngs[0]
    base = png.with_suffix("")
    pal_rel = base.relative_to(root).as_posix() + ".gbapal"
    gfx_rel = base.relative_to(root).as_posix() + ".4bpp.smol"

    print("\nLULLABIES TO PARALYZE:")
    print("  SOURCE PNG ->", png.relative_to(root).as_posix())
    print("  MAKE GERA  ->", pal_rel)
    print("  MAKE GERA  ->", gfx_rel)
...
def main():
    ...
    bdir = root / "PHYTON/backups" / f"no_one_knows_guitar_covers_v2_1_{stamp}"
    ...
    try:
        ...
        print("\n============================================================")
        print("NO ONE KNOWS V2.1 + QOTSA COVERS INSTALADO")
        print("============================================================")
        print("NO ONE KNOWS:")
