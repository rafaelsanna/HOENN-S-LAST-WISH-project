...
def voice_score(line,role):
    ...
    if role=="violin":
        # Strongly prefer a literal violin/solo-string voice of ANY voice type
        # (directsound, keysplit, etc). This is safe because the referenced
        # symbols already exist globally in the sound engine.
        score=0
        for tok,pts in (
            ("solo_violin",900),("violin",850),("fiddle",760),
            ("solo_string",620)
        ):
            if tok in s: score+=pts
    ...
    if role=="strings":
        score=0
        for tok,pts in (
            ("string_ensemble",700),("strings_ensemble",700),
            ("slow_strings",620),("warm_strings",600),
            ("strings",500),("string",360),("violin",160)
        ):
            if tok in s: score+=pts
    ...
    if role=="choir":
        score=0
        for tok,pts in (
            ("choir",800),("voice_ooh",750),("voice_oohs",750),
            ("aahs",740),("aah",600),("vocal",560),("chorus",500),
            ("voice",260)
        ):
            if tok in s: score+=pts
...
def build_custom_violin_from_strings(pool):
    # If the project has no literal violin voice, create a NEW voice definition
    # from its best string DirectSound waveform. This is a new instrument entry,
    # not a sax/flute substitution. The waveform is reused, with a sustained
    # bowed-style envelope suitable for violin melody.
    ranked=[]
    for clean,original,p in pool:
        if not clean.startswith("voice_directsound"):
            continue
        sc=voice_score(original,"strings")
        if sc<=0:
            continue
    ...
    # Parse: voice_directsound key, pan, WaveData, attack, decay, sustain, release
    m=re.match(
    ...
    # Fast bow attack, stable sustain and longer release.
    custom=f"voice_directsound 60, 0, {wave}, 255, 0, 240, 180"
    return (custom, original, p)
...
def choose_orchestra_voices(root):
    ...
    # Piano / strings / choir can use any existing compatible voice definition.
    for role in ("piano","strings","choir"):
        ranked=sorted(
        ...
        if pick is None:
            if role=="choir":
                print("\n[AVISO] Nenhum choir reconhecido; usando strings como backing vocal.")
                chosen["choir"]=chosen["strings"]
                continue
            die(f"Nao achei uma voz confiavel para {role}.")

        clean,original,p,sc=pick
        chosen[role]=(clean,original,p)
        used.add(clean)

    # VIOLIN: first try a real violin/fiddle/solo_string of ANY voice_* type.
    ranked=sorted(
        ((voice_score(original,"violin"),clean,original,p)
         for clean,original,p in pool),
        reverse=True,
        key=lambda x:x[0]
    )
    ...
    for sc,clean,original,p in ranked:
        if sc<=0:
            break
        violin_pick=(clean,original,p)
        break

    if violin_pick is not None:
        chosen["violin"]=violin_pick
        violin_mode="voz de violin existente no projeto"
    else:
        violin_pick=build_custom_violin_from_strings(pool)
        if violin_pick is None:
            die(
                "Nao achei violin nem sample DirectSound de strings para criar um violin novo."
            )
        chosen["violin"]=violin_pick
        violin_mode="VIOLIN NOVO criado a partir do melhor sample de strings"

    print("\nPALETA ORQUESTRAL encontrada/criada:")
    for role in ("piano","violin","strings","choir"):
        clean,original,p=chosen[role]
        print(f"  {role.upper():7s} -> {clean}")
        print("           fonte:",p)

    print("\nVIOLIN:",violin_mode)
    if violin_mode.startswith("VIOLIN NOVO"):
        print("  Envelope novo: attack 255 / decay 0 / sustain 240 / release 180")
        print("  Nenhum instrumento de sopro foi usado como substituto.")
...
def main():
    ...
    bdir=root/"PHYTON/backups"/f"anime_orchestra_rock_fix_v3_{stamp}"
    ...
    print(" GIREI: piano + VIOLIN REAL/NOVO + strings, tempo original lento, sem sopro")
    print(" GUTS THEME: piano + violin + CHOIR/backing vocal")
    print(" BROTHERS: piano + VIOLIN REAL/NOVO + strings")
    print("\nCFG:")
