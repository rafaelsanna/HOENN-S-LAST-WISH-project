...
static void DrawWallpaper(const void *tilemap, s8 direction, u8 offset)
{
    ...
    // HLW V20: every player-facing wallpaper now uses one standardized HGSS
    // source format. The installer normalizes authored tilemap palette banks
    // to start at 1, so Emerald's original +3 double-buffer mapping is valid
    // for both wallpaper buffers and cannot depend on the neighbouring box.
