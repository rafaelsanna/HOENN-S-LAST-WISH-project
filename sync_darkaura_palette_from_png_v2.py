#!/usr/bin/env python3
"""
DarkAura palette sync from indexed PNG - V2 (validator fixed).
...
Use from pokeemerald-expansion root:
    python3 sync_darkaura_palette_from_png_v2.py --dry-run
    python3 sync_darkaura_palette_from_png_v2.py
...
def validate_root(root: Path):
    if not (root / "Makefile").is_file():
        raise SyncError("Rode este script na raiz do pokeemerald-expansion.")

    required = [PNG, GRAPHICS_H, MOVEMENT_C]
    missing = [str(rel) for rel in required if not (root / rel).is_file()]
    if missing:
        raise SyncError(
            "Arquivos obrigatorios nao encontrados: " + ", ".join(missing)
        )

    info = parse_indexed_png(root / PNG)

    if (info["width"], info["height"]) != (EXPECTED_W, EXPECTED_H):
        raise SyncError(
            f"darkaura.png esta em {info['width']}x{info['height']}; "
            f"esperado {EXPECTED_W}x{EXPECTED_H}."
        )

    if info["color_type"] != 3:
        raise SyncError(
            "darkaura.png precisa estar em modo Indexed/Paletted (PNG color type 3)."
        )

    # The current 64x64 graphics conversion should already exist. If it does,
    # validate its byte size before touching the palette.
    if (root / GFX).is_file():
        size = (root / GFX).stat().st_size
        if size != EXPECTED_GFX_BYTES:
            raise SyncError(
                f"darkaura.4bpp tem {size} bytes; "
                f"esperado {EXPECTED_GFX_BYTES} (0x{EXPECTED_GFX_BYTES:X})."
            )
