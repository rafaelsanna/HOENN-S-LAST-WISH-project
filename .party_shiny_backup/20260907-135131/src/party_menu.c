...
#define PARTY_SHINY_TEXT_FG     13
#define PARTY_SHINY_TEXT_SHADOW 14
...
static void ApplyPartyShinyTextPalette(void)
{
    ...
    // Party-slot palettes are 3..8. Entries 13/14 are outside the slot-art,
    // HP-bar and gender remap sets, so they are safe dedicated text colors.
    // These exactly match the pale-yellow Shiny name used by the Summary Screen.
    for (pal = 3; pal <= 8; pal++)
    {
        gPlttBufferUnfaded[BG_PLTT_ID(pal) + PARTY_SHINY_TEXT_FG] = RGB(31,30,16);
        gPlttBufferUnfaded[BG_PLTT_ID(pal) + PARTY_SHINY_TEXT_SHADOW] = RGB(15,12,4);
    }
}
