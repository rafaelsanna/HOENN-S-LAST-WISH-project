...
static void LoadWallpaperGfx(u8 boxId, s8 direction)
{
    ...
    if (wallpaperId != WALLPAPER_FRIENDS)
    {
        ...
        // HLW: always update both the unfaded and currently displayed palette.
        // The storage VBlank uses gPlttBufferFaded for its partial transfers, so
        // copying only gPlttBufferUnfaded on the initial (direction == 0) load
        // leaves Box 1 showing stale wallpaper colors until the first box scroll.
...
static void DrawWallpaper(const void *tilemap, s8 direction, u8 offset)
{
    ...
    // HLW: wallpaper assets from different storage generations do not all use
    // the same palette numbers in their authored tilemaps. Emerald's original
    // assets typically use source banks 1/2, while the imported HGSS-style
    // assets use 0/1 (Torchic uses bank 0 only). The two live wallpaper
    // buffers, however, are always loaded into BG banks 4/5 and 6/7.
    //
    // Normalize the authored tilemap so its first used palette lands on the
    // first palette bank of the active wallpaper buffer. This also prevents
    // one box from accidentally reading the stale palette of its neighbour.
    for (i = 0; i < 20 * 18; i++)
    {
        u8 pal = (map[i] >> 12) & 0xF;
        if (pal < minSourcePal)
    ...
    paletteNum = (offset * 2) + 4 - minSourcePal;
