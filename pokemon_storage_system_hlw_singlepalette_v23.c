...
static void DrawWallpaper(void *, s8, u8);
...
static void LoadWallpaperGfx(u8 boxId, s8 direction)
{
    ...
    if (wallpaperId != WALLPAPER_FRIENDS)
    {
        ...
        // HLW V23 wallpaper contract: every player-facing wallpaper is one
        // 4bpp tileset + one 16-color palette. Each double-buffered wallpaper
        // gets its own fixed palette bank (4 or 6). Source tilemap palette bits
        // are ignored by DrawWallpaper, so custom art can be authored with
        // palette 0, 1, or any stale editor value without ever touching bank 3.
        LoadPalette(wallpaper->palettes,
                    BG_PLTT_ID(4 + sStorage->wallpaperOffset * 2),
                    PLTT_SIZE_4BPP);
    ...
    else
    {
        ...
        DecompressDataWithHeaderWram(wallpaper->tilemap, sStorage->wallpaperTilemap);
        DrawLegacyWallpaper(sStorage->wallpaperTilemap, sStorage->wallpaperLoadDir, sStorage->wallpaperOffset);
        ...
        // Same rule for Walda wallpapers: keep faded/unfaded buffers in sync
        // even on the first box shown after entering storage.
...
static void DrawWallpaper(void *tilemap, s8 direction, u8 offset)
{
    u16 *map = tilemap;
    u16 i;
    s16 tileOffset = offset * 256;
    s16 paletteNum = 4 + (offset * 2);
    s16 x = ((sStorage->bg2_X / 8 + 10) + (direction * 24)) & 0x3F;

    // HLW V23: player-facing wallpapers are intentionally palette-agnostic.
    // Preserve tile index + H/V flip bits, but discard the palette nibble that
    // came from the authored tilemap. The engine owns palette placement:
    //   wallpaper buffer 0 -> BG palette 4
    //   wallpaper buffer 1 -> BG palette 6
    // Palette 3 belongs exclusively to the selectable PC background. This is
    // what makes edited one-sheet wallpapers independent from the neighboring
    // box and prevents them from recoloring/corrupting the animated PC BG.
    for (i = 0; i < 20 * 18; i++)
        map[i] &= 0x0FFF;
...
static const u8 *const sMenuTexts[] =
{
    ...
    [MENU_SCENERY_1]  = COMPOUND_STRING("HGSS 1"),
    [MENU_SCENERY_2]  = COMPOUND_STRING("HGSS 2"),
    [MENU_ETCETERA_1] = COMPOUND_STRING("HGSS 3"),
    [MENU_ETCETERA_2] = COMPOUND_STRING("HGSS 4"),
    ...
    [MENU_TIMEANDSPACE] = COMPOUND_STRING("TIME & SPACE"),
