...
#define POKEDEX_SEARCH_FORCE_DARK TRUE
...
static void PrintSearchTextToFit(const u8 *str, u32 x, u32 y, u32 width)
{
    ...
    {
        // The original Search tileset does not use pixel values 4 or 15.
        // Reserve them for dynamic white text + dark shadow in dark mode.
        color[1] = 4;
        color[2] = 15;
    }
    else
...
{
    if (POKEDEX_SEARCH_FORCE_DARK)
        FillWindowPixelRect(0, PIXEL_FILL(3), x, y, width, height);
    else
        FillWindowPixelRect(0, PIXEL_FILL(0), x, y, width, height);
}


// The original Search tileset is kept byte-for-byte on disk.
// Dark-mode-only semantic remapping is done after it is decompressed to VRAM.
//
// Original tileset semantics:
//   pixel 3  = white button lettering AND white light-mode surfaces
//   pixel 8/9  = green button fill/shadow
//   pixel 10/11 = parameter-button fill/shadow
//
// Dark Search already needs pixel 3 to be charcoal for the surfaces, so button
// lettering cannot stay on pixel 3. Pixel 4 is unused by the original Search
// tileset, so remap button lettering 3 -> 4 in VRAM only.
...
static void LoadSearchDarkSemanticPalette(void)
{
    ...
    static const u16 sGreen[4][2] =
    {
        { RGB(14, 24, 13), RGB(4, 11, 6) },
        { RGB(8, 15, 8),   RGB(4, 7, 4)  },
        { RGB(7, 10, 7),   RGB(4, 5, 4)  },
        { RGB(5, 7, 5),    RGB(3, 4, 3)  },
    };

    static const u16 sBlue[4][2] =
    {
        { RGB(11, 25, 28), RGB(4, 10, 12) },
        { RGB(6, 12, 14),  RGB(4, 7, 8)   },
        { RGB(9, 13, 14),  RGB(6, 8, 9)   },
        { RGB(5, 8, 9),    RGB(4, 5, 6)   },
    };

    static const u16 sRed[4][2] =
    {
        { RGB(27, 15, 17), RGB(15, 6, 8) },
        { RGB(22, 11, 13), RGB(12, 6, 7) },
        { RGB(18, 12, 13), RGB(11, 7, 8) },
        { RGB(13, 8, 9),   RGB(8, 6, 6)  },
    };

    static const u16 sDark = RGB(4, 4, 5);
    static const u16 sOutside = RGB(6, 6, 7);
    static const u16 sWhite = RGB_WHITE;
    static const u16 sShadow = RGB(8, 8, 8);

    for (state = 0; state < 4; state++)
    {
        u8 greenBank = 4 + state;
        u8 redBank   = 8 + state;
        u8 blueBank  = 12 + state;

        // Keep background/window banks 0..3 dark.
        LoadPalette(&sDark, BG_PLTT_ID(state) + 0, PLTT_SIZEOF(1));
        LoadPalette(&sDark, BG_PLTT_ID(state) + 3, PLTT_SIZEOF(1));
        LoadPalette(&sWhite, BG_PLTT_ID(state) + 4, PLTT_SIZEOF(1));
        LoadPalette(&sShadow, BG_PLTT_ID(state) + 15, PLTT_SIZEOF(1));

        // Start each semantic button bank from the matching original state.
        LoadPalette(sPokedexPlusHGSS_MenuSearch_dark_Pal + state * 16,
                    BG_PLTT_ID(greenBank), PLTT_SIZEOF(16));
        LoadPalette(sPokedexPlusHGSS_MenuSearch_dark_Pal + state * 16,
                    BG_PLTT_ID(redBank), PLTT_SIZEOF(16));
        LoadPalette(sPokedexPlusHGSS_MenuSearch_dark_Pal + state * 16,
                    BG_PLTT_ID(blueBank), PLTT_SIZEOF(16));

        // Shared semantic rules for all button families:
        // 3 = white glyph, 6 = dark outside/background pixels.
        LoadPalette(&sWhite, BG_PLTT_ID(greenBank) + 3, PLTT_SIZEOF(1));
        LoadPalette(&sWhite, BG_PLTT_ID(redBank) + 3, PLTT_SIZEOF(1));
        LoadPalette(&sWhite, BG_PLTT_ID(blueBank) + 3, PLTT_SIZEOF(1));

        LoadPalette(&sWhite, BG_PLTT_ID(greenBank) + 4, PLTT_SIZEOF(1));
        LoadPalette(&sWhite, BG_PLTT_ID(redBank) + 4, PLTT_SIZEOF(1));
        LoadPalette(&sWhite, BG_PLTT_ID(blueBank) + 4, PLTT_SIZEOF(1));

        LoadPalette(&sOutside, BG_PLTT_ID(greenBank) + 6, PLTT_SIZEOF(1));
        LoadPalette(&sOutside, BG_PLTT_ID(redBank) + 6, PLTT_SIZEOF(1));
        LoadPalette(&sOutside, BG_PLTT_ID(blueBank) + 6, PLTT_SIZEOF(1));

        LoadPalette(&sShadow, BG_PLTT_ID(greenBank) + 15, PLTT_SIZEOF(1));
        LoadPalette(&sShadow, BG_PLTT_ID(redBank) + 15, PLTT_SIZEOF(1));
        LoadPalette(&sShadow, BG_PLTT_ID(blueBank) + 15, PLTT_SIZEOF(1));

        // GREEN family: SEARCH / SHIFT / OK.
        LoadPalette(&sGreen[state][0], BG_PLTT_ID(greenBank) + 8, PLTT_SIZEOF(1));
        LoadPalette(&sGreen[state][1], BG_PLTT_ID(greenBank) + 9, PLTT_SIZEOF(1));
        LoadPalette(&sGreen[state][0], BG_PLTT_ID(greenBank) + 10, PLTT_SIZEOF(1));
        ...
        LoadPalette(&sRed[state][1], BG_PLTT_ID(redBank) + 9, PLTT_SIZEOF(1));
        LoadPalette(&sRed[state][0], BG_PLTT_ID(redBank) + 10, PLTT_SIZEOF(1));
        LoadPalette(&sRed[state][1], BG_PLTT_ID(redBank) + 11, PLTT_SIZEOF(1));
        LoadPalette(&sRed[state][0], BG_PLTT_ID(redBank) + 12, PLTT_SIZEOF(1));
        LoadPalette(&sRed[state][1], BG_PLTT_ID(redBank) + 13, PLTT_SIZEOF(1));
        LoadPalette(&sRed[state][1], BG_PLTT_ID(redBank) + 14, PLTT_SIZEOF(1));

        // BLUE family: NAME / COLOR / TYPE / ORDER / MODE + their selection boxes.
        LoadPalette(&sBlue[state][0], BG_PLTT_ID(blueBank) + 8, PLTT_SIZEOF(1));
        LoadPalette(&sBlue[state][1], BG_PLTT_ID(blueBank) + 9, PLTT_SIZEOF(1));
        LoadPalette(&sBlue[state][0], BG_PLTT_ID(blueBank) + 10, PLTT_SIZEOF(1));
        LoadPalette(&sBlue[state][1], BG_PLTT_ID(blueBank) + 11, PLTT_SIZEOF(1));
        LoadPalette(&sBlue[state][0], BG_PLTT_ID(blueBank) + 12, PLTT_SIZEOF(1));
        LoadPalette(&sBlue[state][1], BG_PLTT_ID(blueBank) + 13, PLTT_SIZEOF(1));
        LoadPalette(&sBlue[state][1], BG_PLTT_ID(blueBank) + 14, PLTT_SIZEOF(1));
    }
}
...
static void SetSearchRectHighlight(u8 flags, u8 x, u8 y, u8 width)
{
    ...
    if (y == 0 && width == 5)
    {
        // Top bar.
        if (x == 12)
            paletteBank = 8 + flags;  // CANCEL
        else
            paletteBank = 4 + flags;  // SEARCH / SHIFT
    }
    else if (x == 0 && width == 5
          && ((!IsNationalPokedexEnabled() && y == 10)
           || ( IsNationalPokedexEnabled() && y == 12)))
    {
        paletteBank = 4 + flags;      // OK
    }
    else
    {
        paletteBank = 12 + flags;     // NAME / COLOR / TYPE / ORDER / MODE
    }
