...
static void Task_MainMenuTurnOff(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        // Zera gPlttBufferFaded agora, enquanto VBlankCB ainda está ativo.
        // Sem isso, o próximo TransferPlttBuffer() copia cores residuais
        // do menu (lilás/rosa) para o hardware antes do próximo callback
        // ter chance de carregar as paletas corretas.
...
static bool8 MainMenu_InitBgs(void)
{
    ...
    // ShowBg calls moved to case 6 of DoGfxSetup, after palettes are already black
    return TRUE;
}
