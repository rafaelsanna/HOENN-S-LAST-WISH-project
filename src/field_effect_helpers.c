...
static bool8 IsSeaGrassMetatile(s16 x, s16 y)
{
    return MapGridGetMetatileIdAt(x, y) == METATILE_Cave_sea_grass;
}
...
u8 FindDryGrassFieldEffectSpriteId(u8 localId, u8 mapNum, u8 mapGroup, s16 x, s16 y)
{
    ...
    for (i = 0; i < MAX_SPRITES; i ++)
    {
        ...
        {
            struct Sprite *sprite = &gSprites[i];
            if (sprite->callback == UpdateDryGrassFieldEffect
                && (x == sprite->sX && y == sprite->sY)
                && localId == (u8)(sprite->sLocalId)
                && mapNum == (sprite->sMapNum & 0xFF)
                && mapGroup == sprite->sMapGroup)
...
u8 FindFairyGrassFieldEffectSpriteId(u8 localId, u8 mapNum, u8 mapGroup, s16 x, s16 y)
{
    ...
    for (i = 0; i < MAX_SPRITES; i ++)
    {
        ...
        {
            struct Sprite *sprite = &gSprites[i];
            if (sprite->callback == UpdateFairyGrassFieldEffect
                && (x == sprite->sX && y == sprite->sY)
                && localId == (u8)(sprite->sLocalId)
                && mapNum == (sprite->sMapNum & 0xFF)
                && mapGroup == sprite->sMapGroup)
...
u8 FindSeaGrassFieldEffectSpriteId(u8 localId, u8 mapNum, u8 mapGroup, s16 x, s16 y)
{
    ...
    for (i = 0; i < MAX_SPRITES; i ++)
    {
        ...
        {
            struct Sprite *sprite = &gSprites[i];
            if (sprite->callback == UpdateSeaGrassFieldEffect
                && (x == sprite->sX && y == sprite->sY)
                && localId == (u8)(sprite->sLocalId)
                && mapNum == (sprite->sMapNum & 0xFF)
                && mapGroup == sprite->sMapGroup)
...
u32 FldEff_DryGrass(void)
{
    ...
    spriteId = CreateSpriteAtEnd(
        gFieldEffectObjectTemplatePointers[FLDEFFOBJ_DRY_GRASS],
        x, y, 0);
...
void UpdateDryGrassFieldEffect(struct Sprite *sprite)
{
    ...
    if (TryGetObjectEventIdByLocalIdAndMap(localId, mapNum, mapGroup, &objectEventId)
     || !MetatileBehavior_IsDryGrass(metatileBehavior)
     || (sprite->sObjectMoved && sprite->animEnded))
    {
        FieldEffectStop(sprite, FLDEFF_DRY_GRASS);
    }
    else
    {
        struct ObjectEvent *objectEvent = &gObjectEvents[objectEventId];
...
u32 FldEff_FairyGrass(void)
{
    ...
    spriteId = CreateSpriteAtEnd(
        gFieldEffectObjectTemplatePointers[FLDEFFOBJ_FAIRY_GRASS],
        x, y, 0);
...
void UpdateFairyGrassFieldEffect(struct Sprite *sprite)
{
    ...
    if (TryGetObjectEventIdByLocalIdAndMap(localId, mapNum, mapGroup, &objectEventId)
     || !MetatileBehavior_IsFairyGrass(metatileBehavior)
     || (sprite->sObjectMoved && sprite->animEnded))
    {
        FieldEffectStop(sprite, FLDEFF_FAIRY_GRASS);
    }
    else
    {
        struct ObjectEvent *objectEvent = &gObjectEvents[objectEventId];
...
u32 FldEff_SeaGrass(void)
{
    ...
    spriteId = CreateSpriteAtEnd(
        gFieldEffectObjectTemplatePointers[FLDEFFOBJ_SEA_GRASS],
        x, y, 0);
...
void UpdateSeaGrassFieldEffect(struct Sprite *sprite)
{
    ...
    if (TryGetObjectEventIdByLocalIdAndMap(localId, mapNum, mapGroup, &objectEventId)
     || !IsSeaGrassMetatile(sprite->sX, sprite->sY)
     || (sprite->sObjectMoved && sprite->animEnded))
    {
        FieldEffectStop(sprite, FLDEFF_SEA_GRASS);
    }
    else
    {
        struct ObjectEvent *objectEvent = &gObjectEvents[objectEventId];
        ...
        UpdateObjectEventSpriteInvisibility(sprite, FALSE);
        UpdateGrassFieldEffectSubpriority(sprite, sprite->sElevation, subpriority);
    }
}

u32 FldEff_JumpDryGrass(void)
{
    u8 spriteId;
    ...
    spriteId = CreateSpriteAtEnd(
        gFieldEffectObjectTemplatePointers[FLDEFFOBJ_JUMP_DRY_GRASS],
    ...
    if (spriteId != MAX_SPRITES)
    {
        ...
        sprite->oam.priority = gFieldEffectArguments[3];
        sprite->sJumpElevation = gFieldEffectArguments[2];
        sprite->sJumpFldEff = FLDEFF_JUMP_DRY_GRASS;
    }
